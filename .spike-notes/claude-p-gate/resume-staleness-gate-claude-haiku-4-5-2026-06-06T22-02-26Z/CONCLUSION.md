...
2. **Transcript-growth gate (core):** at echo-confirm (just before submit) snapshot `baseline_turns = turnCountFile(transcript)`. After Stop, the post-Stop parse loop accepts a result **only once `num_turns > baseline_turns`** (the live turn appended a new assistant turn). The Stop-payload fallback is used only when growth is confirmed (text-flush lag) or on a fresh turn (`baseline == 0`, no prior to be stale). If the count never grows → the Stop was stale/replayed → `StopTimeout` (refuse to emit the prior answer; the bridge cold-retries).
