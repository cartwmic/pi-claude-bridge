...
**Choice:** Add `driver` to in-memory cache and sidecar schema. Legacy missing field decodes as `claude-p`; invalid field is malformed/cold. Mismatch invalidates and cold-starts. Sidecar key/history/version rules stay unchanged. Track attempt phase `spawned → ready → promptSubmitted → turnAccepted → terminal`; `turnAccepted` requires first top-level model `message_start`, not a successful stdin write. Persist selected driver/history for non-error main completion, including direct abort only after `turnAccepted`. Abort before acceptance preserves prior validated hint/sidecar unchanged; a fresh attempt creates no hint. Thus current user history is never marked as seen before model acceptance evidence. Direct floor >=2.1.208 is checked before child spawn; interactive path keeps independent support.
...
8. Rollout is additive. Operational rollback sets/removes config to `claude-p`. If shared refactor regresses interactive mode, revert change commits/reinstall prior package rather than relying on selection. New reader treats missing field as `claude-p`. Before package downgrade, run new maintenance command `npm run resume:quarantine-direct` to atomically move every `driver:"claude-print"` sidecar into timestamped bridge-owned backup outside active resume directory, verify zero active direct/invalid sidecars, stop pi so in-memory hints vanish, then reinstall/revert package. Old reader may ignore additive fields only after direct hints are quarantined; first downgraded turn must prove cold. Preserve diagnostics and quarantine backup until rollback verification passes.
