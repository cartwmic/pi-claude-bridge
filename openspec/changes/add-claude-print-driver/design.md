...
1. `shim-first`: after D4 has filtered to qualified `mcp__custom-tools__*` observations only, park request, mint pi id, emit one pi tool call, and queue canonical name/args awaiting an order-optional but count-mandatory matching bridged observation; result resolver remains keyed by that pi id for its entire lifetime. Native/housekeeping/foreign observations never enter coordinator counts.
...
4. `batch-close`: assistant `message_stop` seals that observation batch but does not reject a delayed shim request. Direct observations may publish incrementally, while `claude-p` can publish several completed assistant batches together only at terminal result; therefore an unmatched request present when batch N seals is not classified as N overflow before later batches are published. Canonical pairing continues in batch/arrival order. At selected-driver terminal finalization, every sealed observation must be paired and no shim request may remain unmatched; any mismatch transitions to correlation error, drains pending resolvers, and invalidates resume. Fewer shim requests may wait without a new liveness timer until observations arrive, terminal/process failure, or caller abort.
...
Static system-prompt bytes at selected-driver boundary equal caller `ctx.systemPrompt` exactly (text or `--system-prompt-file`). Remove current readiness/capture guard prepends from static system prompt. Readiness is enforced structurally per adapter: direct capture waits on D3 sentinel before writing user NDJSON; interactive capture passes the same sentinel path as `mcpReadyFile` to maintained `claude-p` fork, whose `--mcp-ready-file` gate holds submit Enter until shim serves exact `tools/list`. Neither capture path needs `WaitForMcpServers` in static prompt. Capture forcing/readiness fallback instructions become a bridge-owned, adapter-appropriate user-prompt control suffix after replay, preserving system-prompt fidelity while keeping sole-tool behavior.
...
Before matrix use, make harness itself fail closed: maintain explicit S0–S27 required inventory, convert S21 investigation into deterministic assertions (do not exclude it), make S22/any required precondition skip a nonzero gate failure, assert requested driver actually spawned in every script, and add harness fixtures proving forced scenario failure/skip propagates nonzero. S28 idle-cutoff and S29 mid-held failure run as required driver integration gates. Required gate id `driver-large-cold-start` executes `scripts/run-scenario-s31-large-cold-start-prompt.sh` once with `claude-p` and once with `claude-print`, asserting the >800-byte sentinel/coherent answer for each. S32 remains the sole driver-specific peek expectation.
