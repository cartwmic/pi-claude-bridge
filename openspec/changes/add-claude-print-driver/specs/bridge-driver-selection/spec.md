...
THE bridge SHALL select exactly one inference driver from `claude-p` or `claude-print` using precedence `CLAUDE_BRIDGE_DRIVER` compatibility override, project `<project>/.pi/claude-bridge.json`, global `~/.pi/agent/claude-bridge.json`, then default `claude-p`.
...
#### Scenario: Invalid configured driver fails loud
- **IF** highest-precedence driver configuration contains a value other than `claude-p` or `claude-print`
...
WHEN an invocation creates a frame, THE bridge SHALL pin its selected driver until that frame and any parked tool-result delivery complete.
