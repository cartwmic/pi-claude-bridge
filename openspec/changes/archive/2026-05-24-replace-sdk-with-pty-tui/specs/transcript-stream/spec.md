...
Tails the inference driver's transcript JSONL during an in-flight turn and
emits structured events to the bridge's stream layer. Replaces the SDK
iterator's per-event interface with a JSONL-tail equivalent. The transcript
path is computed deterministically by the driver from the pre-generated
`--session-id` UUID and the encoded cwd (D18); the tailer opens the path as
soon as the file appears on disk via a parent-directory `fs.watch` for
file-creation event. The `SessionStart` hook serves as a confirmation cross-
check only, not as the discovery mechanism.
...
|---|---|---|---|---|---|
| transcript-stream.tail-transcript-while-turn-is-in-flight | [ ] | [ ] | [ ] | [ ] | [ ] |
...
| transcript-stream.malformed-jsonl-lines-surface-as-warnings-not-stream-errors | [ ] | [ ] | [ ] | [ ] | [ ] |
| transcript-stream.missing-or-unreadable-transcript-surfaces-as-error | [ ] | [ ] | [ ] | [ ] | [ ] |
