...
**Persist only on the main-turn finalize:** `finalizeClaudePFrame` (`index.ts:1371`) runs for **every** frame on the stack — including subagent frames, which carry their own `sessionId` and reach the cache-set branch (`index.ts:1402`). The cache-set branch (1402-1408) is itself **unguarded** — it runs for every frame. So the fix is to add a **new** main-turn check **around the `writeSidecar` call**. **The discriminator is `stack[0] === frame`, NOT `top() === frame`** (correction made at apply time, 2026-06-06): the main-provider turn is always the **bottom** of the stack (`stack[0]`); a subagent nests **above** it and is itself `top()` at its own finalize — so `top() === frame` is true for a subagent too and would NOT exclude it. `stack[0] === frame` holds only for the main turn (verified against the stack model at `index.ts:458-459`: subagents nest a child query in a parent's blocked tool-handler slot, so the main turn is `stack[0]`). Without this gate a subagent finalize would record a subagent's session and a later `--resume` would reattach the wrong transcript.
