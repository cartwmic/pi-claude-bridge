...
- **Outstanding (pre-apply):** task 0.3 (land fork gate + repin) is DONE (2026-06-06). New item from Check 13 — **Risk R7 / D2(c): warm-safety must require "`claude` saw every prefix message," not just a prefix-match** (the cross-provider missed-messages invariant). The matching pre-existing `syncSharedSession` bug is a separate fix (captured in mcp-memory). Step-6 owner calls resolved: no kill-switch; constitution bump MAJOR.
...
## Check 13 — Cross-provider missed-messages: warm-safety refinement + a separate pre-existing bug (2026-06-06)

Landing the gate made `int-session-resume.mjs` runnable again (its `.env.test` alt provider was a dead model; fixed to `openai-codex/gpt-5.4`), which immediately surfaced a real, **deterministic, pre-existing** bug: on Turn 4 the bridge warm-resumes (`resume=e7c8a601 msgs=7`) a `claude` session that last saw up to Turn 2, dropping the Turn-3 word set on the **codex** turn (`claude` answers with only the 2 words it saw). The resume-staleness gate is **verified working** here (Turn-4 resume-diag `staleSuspected:false`, live `out=550` generation) — this is NOT the stale-result race; it's `syncSharedSession` treating a longer pi history as a benign prefix-extension without noticing the intervening messages were never sent to `claude`.

- **No bridge runtime code changed this session** (the 7 commits are docs + spike notes + a 2-line pin), so this is not introduced by the landing.
- **Captured** as a standalone bug (mcp-memory). Fix direction: rebuild/cold-start (or inject the missed messages) whenever the `claude` session is missing ANY prefix message, regardless of provider.
- **Fed into this change** (Risk R7, D2(c), spec R3 clause + "Unseen intervening messages force cold" scenario): warm-safety requires "`claude` saw every message in the prefix," not just a prefix-match — the same invariant.
