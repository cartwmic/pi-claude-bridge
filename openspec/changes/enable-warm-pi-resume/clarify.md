...
| A4 | warm-pi-resume:R5 | What is the stale discriminator — a turn-counter delta, or the driver's self-contained `staleSuspected`? | A turn-counter (`num_turns`) delta vs. a baseline | The driver's `staleSuspected` (replay boundary seen, no live prompt after) | **superseded** | **B (revised in adversarial review)** — `staleSuspected` is self-contained (no baseline needed) and is the in-tree primary discriminator (`stream.ts:57-80`); `num_turns` is a weaker corroborator. The original "which baseline" question is moot under `staleSuspected`. See design D5. |
...
| C3 | two pi sessions, same cwd, concurrently | Sidecar key collision? | Keyed by literal `spawnCwd` + full `sessionId` → distinct keys; invariant 1 holds | — | answered | **A** — distinct pi sessionIds → distinct sidecars; no collision. Same-session double-process: tolerated via atomic (temp+rename) writes — last writer wins; a torn read falls back to cold (R4 corrupt-sidecar). |
| C4 | resume + sidecar valid BUT claude transcript file deleted/cleaned | Pre-check transcript existence, or rely on `--resume` failing → cold? | Rely on `--resume` failure → driver-error/stale guard → cold retry (no extra `~/.claude` read) | Pre-stat the deterministic-path transcript (Principle III(b)-bounded) | **resolved → B (adversarial review)** | The silent-fresh branch produces a context-free live turn the D5 stale guard CANNOT catch (the turn runs), so option A is unsafe. Commit the III(b) existence pre-check → fail-closed cold (R4 + "Unconfirmable transcript" AC). The T0.1 spike stays but only characterizes claude's behavior; it no longer gates safety. |
| C5 | warm-resume implemented before stale-result enforcement lands | Can R5's guard stand alone, or must the dependency land first? | This change implements only the per-resume `staleSuspected` guard; broad in-process stale-enforcement is the named dependency | Fold full stale-enforcement into this change | **deferred** | Sequencing/scope risk. The per-resume `staleSuspected` guard (corrected from `num_turns` + requiring the 3.3 plumbing, design D5) is implementable here; standalone is conditional on that being load-bearing. The broader enforcement is a separate change. Echo to analyze for owner triage. |

**Status summary:** 0 unanswered (design unblocked). 1 deferred (C5 sequencing) → carried into `analyze.md` outstanding-risks. C4 resolved to fail-closed during adversarial review (the T0.1 spike remains as characterization, not a safety gate). A4 superseded (`staleSuspected` discriminator); C3b added (full sessionId).
