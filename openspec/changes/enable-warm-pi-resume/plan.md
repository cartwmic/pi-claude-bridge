...
- **Action (5-step micro-tasks):**
  1. **Write failing tests** `tests/unit-warm-resume-gate.mjs` citing:
       - `warm-pi-resume.validated-warm-resume-on-pi-resume` — `sha256` prefix-extension match + version match + no unseen intervening messages (only the new turn appended, R7) → `{ warm: true }`; a prefix-extension with unseen intervening messages (provider switch) → `{ warm: false }`.
