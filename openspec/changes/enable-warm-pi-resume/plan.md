...
- **Covers:** T1.1 (constitution — Principle I + III(b) body + III Enforcement), T1.2 (domain), T1.3 (III CI audit test)
- **Pre-conditions:** Step 0 complete (the amendment text references the spike outcomes); `openspec/constitution.md` + `openspec/domain.md` + `tests/int-claude-dir-audit.mjs` present.
- **Action (ordered — doc + CI-test edits, not feature TDD):**
  1. **Principle I** — permit a **content-free** resume-metadata sidecar (fingerprints + ids + version; never message content, no counters; the fingerprint chain is a one-way digest) stored **outside `~/.claude/`**.
  2. **Principle III(b) body + Enforcement clause** — widen to permit (a) deriving the transcript path from a session id the bridge recorded in its own *prior-session* sidecar and (b) an existence-only `stat` of it (no content read); AND amend the III **Enforcement** clause (`constitution.md:70-74`) — which today only blesses current-PTY `--session-id` reads / hook payloads — to cover that stat. Without this, R4/R8's "III(b) permits" claim is false AND the CI audit fails (Round-2 P1 / Round-3 P1).
  3. **III CI audit (T1.3)** — update `tests/int-claude-dir-audit.mjs` to permit the one blessed existence-`stat` (still rejecting writes + content reads); add `src/resume-store.ts` to its `PROD_FILES`.
  4. **Domain invariant 3** (`domain.md:40-43`) — remove `restart` from the unconditional cold-start list; replace with "restart **without a validated resume sidecar** → cold-start." Bump domain.md version. (Round-2 P0: today's text contradicts the change.)
  5. Bump constitution version — **MAJOR** (partial reversal of Principle I per the Versioning rule); pin the exact version + changelog lines.
  6. Commits: `docs(constitution): permit resume sidecar + widen III(b)/Enforcement (MAJOR)`, `test(audit): bless prior-session transcript stat`, `docs(domain): invariant 3 — validated warm resume on restart`.
- **Verification:**
  - `openspec validate --strict` (constitution + domain parse; versions bumped).
  - Manual: Principle I scopes to fingerprints only (no language admitting message content); III(b) widening is existence-stat-only; invariant 3 no longer asserts unconditional restart→cold.
- **Rollback:** `git revert` the doc commits; restore prior Principle I/III + invariant 3 text + versions.

## Plan step 2: Resume-store module + transcript-existence helper (D1, D3) — TDD

- **Covers:** T2.1 (failing tests), T2.2 (store impl), T2.3 (prune-on-read), T2.4 (transcript-existence helper)
...
- **Action (5-step micro-tasks):**
  1. **Write failing tests** `tests/unit-resume-store.mjs` citing:
     - `warm-pi-resume.resume-sidecar-persisted-on-successful-turn` — write/read round-trip returns the same `{ claudeSessionId, spawnCwd, piSessionId, historyHashChain, claudeVersion }` (NO `lastNumTurns` — the stale guard is self-contained `staleSuspected`).
     ...
     - the transcript-existence helper (T2.4): encodes `~/.claude/projects/<encoded spawnCwd>/<sessionId>.jsonl` from the LITERAL `spawnCwd`, replacing `/` AND `.` (and any non-alnum run) with `-` (verified: `.spike-notes` → `--spike-notes`); test a dotted-path cwd; present→ok, missing→cold.
  2. **Run** `npm run test:unit -- unit-resume-store.mjs` → **expect FAIL** (module absent).
  3. **Minimal impl** `src/resume-store.ts`: `readSidecar(cwd, sessionId)`, `writeSidecar(...)`, `invalidate(...)`, `transcriptPresent(sidecar)` over `~/.pi/agent/resume/<key>.json`; key = literal `frame.cwd` + full `sessionId`; atomic writes (temp+rename); prune-on-read; the writer accepts only the typed fingerprint schema (no content field exists to leak).
...
- **Observed Failure (greenfield):** RED from step 2 — `Cannot find module '../src/resume-store.ts'` / assertion failures on round-trip + content-free + alias-distinctness + prefix-distinctness + existence-encoding.
...
- **Action (5-step micro-tasks):**
  1. **Write failing tests**:
     - `tests/unit-warm-resume-gate.mjs` citing:
       - `warm-pi-resume.validated-warm-resume-on-pi-resume` — `sha256` prefix-extension match + version match + transcript-present → `{ warm: true }`.
