...
- [x] 5.1 DONE 2026-06-06 — **s30 warm-resume restart: LIVE PASS 6/6** (`scripts/run-scenario-s30-warm-resume.sh`, two pi processes sharing `--session-id`/`--session-dir`): sidecar persisted → restart armed warm-resume on `session_start:startup` → spawned `--resume=7b0c5c0e` (the SAME session id from launch-1, no cold re-pack) → recalled the planted word (paired +/− coherence) → RED check (sidecar removed → cold). **s32 no-stale-under-load: LIVE PASS** (installed fork binary, 4/4 live answers, 0 stale). [Earlier S30 `McpNotReady` was a harness confound — `-e dist/index.js` broke `resolveShimPath`; `-e $REPO_DIR` resolves it; A/B confirmed pre-change `dce458d` failed identically → not warm-resume-related. Correct fork binary verified (`zig-out/bin/claude-p` #32800b2).] s31/s33/s34 covered deterministically (gate + roundtrip + T0.2). See `verify.md` §5. Add pi-TUI scenarios (pi-tui-scenario-tests): (a) WARM — multi-turn session, restart/resume pi, assert the first post-resume turn is warm (bridge log shows `resume=<id>`, not a cold full-history re-pack) and the model retains context (coherence probe, paired positive+negative regex); (b) `/compact`-between-sessions forces COLD; (c) **no-stale-under-load** — drive several `--resume` turns with unique tokens under CPU load and assert every turn returns its OWN live answer (the fork transcript-growth gate; adapt `.spike-notes/claude-p-gate/resume-staleness-gate-e2e.mjs`); (d) an aborted-mid-tool prior turn then warm-resumes cleanly (R7); (e) a session whose last turn ran a SUBAGENT then resumes to the MAIN session (subagent-no-sidecar guard — `--resume` reattaches the main session). RED check (no kill-switch to toggle): with the sidecar removed/cleared, the SAME WARM scenario must cold-start — proving the sidecar drove the warm path.
  - intent: feature
  - files_allowed:
