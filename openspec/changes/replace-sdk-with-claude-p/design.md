...
- **G5 abort coherence (S7/S8/S13) — ✅ CLEARED bridge-side (2026-06-02).** The warm-resume RE-ECHO bug is fixed in `stream.ts` (`suppressResumeReplay` option, wired `= useResume` in index.ts): on a resume turn the parser BUFFERS content events and discards the buffer on each real user-PROMPT line (prompt lines = STRING `message.content`; tool_result lines = ARRAY with `tool_result` blocks, which do NOT reset), flushing only the final segment (the live turn) at the terminal `result` (and on `endOfStream` for aborts, so the committed partial is the live text). The "last-segment" rule is immune to count-drift (abort) and duplicate-prompt-text — both of which failed as naïve boundaries. Verified: 230 unit tests (+7 new), real multi-turn warm-resume shows ONLY the live response (no re-echo), G5 S7/S8 re-run 2 pass/0 fail. NO claude-p fork needed. **Remaining S7 nuance (documented-exemption candidate, NOT the re-echo bug):** the LITERAL "what number did you reach before I interrupted?" recall is unsatisfiable because claude-p `--print` buffers the whole turn's text into ~one burst (no token-granular mid-stream partial to capture); abort MECHANICS + "was I interrupted" coherence pass, only the exact-number recall doesn't. Pre-existing claude-p behavior, orthogonal to this fix; carry as a documented S7 exemption per the acceptance bar (record in SCENARIO_RESULTS + T4.1). HISTORICAL framing below. Mechanics PASS (T1.13 clean abort 7–12ms no orphan; T1.14 late-tool-result capture works). Coherence FAILS: discovered claude-p 0.1.0 drains the FULL replayed transcript on `--resume`, so `stream.ts` re-emits every prior assistant text block as new `text-delta` (proven from `g4-singleshot-raw/e1.raw.txt`: turn N emits all prior responses + the new one). This corrupts EVERY warm-resume turn (S0/S6/S12), and the "committed partial" the abort path saves is therefore a stale echo → S7 recall fails, S8 sometimes FABRICATES "tool completed". This is the real last blocker. Two fix paths (`g5-warmresume-reecho.md`): (1) bridge-side stream filtering — emit only the live turn, suppressing the replayed prefix (gate until the new real-user-prompt line, counting `priorUserPromptCount`); (2) claude-p fork — drain only post-`--resume` transcript lines (clean at source; composes with the anticipated persistence fork). G5 clears once the committed partial is the real live-turn text, not a re-echo. OWNER decision on fix path pending.
