# claude-tui-driver Specification
...
WHEN a fresh PTY is spawned for a pi user turn, THE driver SHALL deliver to `claude` a single typed user message that bundles (a) the pi system prompt content wrapped in `<system_context>...</system_context>` tags, immediately followed by (b) the pi user prompt. The bundled message SHALL be typed into the TUI input field after the `SessionStart` hook fires and after the PTY output stream has been silent for an implementation-defined quiescence window (default 80ms; ceiling 2000ms). THE driver SHALL NOT pass the prompt as a positional CLI argument NOR shall it pass `--system-prompt[-file]` or `--append-system-prompt[-file]` flags. THE driver SHALL write the bundled message bytes followed by a separate `\r` keystroke (with an implementation-defined inter-write debounce, default 120ms, to evade Ink's bracketed-paste burst-merging that otherwise lands `\r` in the input buffer instead of triggering submit).

On cold-start (no cached driver session id), the typed prompt segment carries the flattened pi history per the bridge's existing `buildColdStartPrompt` conversion contract. On warm-resume (cached driver session id valid), the typed prompt segment carries only the new user message. The `<system_context>` segment carries `opts.systemPrompt` verbatim on both paths. THE driver SHALL NOT write the prompt to any persistent location outside the PTY's own transcript.

**Rationale (D26+D27):** the positional `[prompt]` form on a fresh `claude` invocation triggers an internal headless-auto-submit code path whose request shape is rejected by Anthropic's OAuth interactive-mode tier cap (`API Error: 400 "out of extra usage"`) when combined with a substantive `--system-prompt` (D26 finding). Subsequent investigation (D27) found that ANY substantive `--system-prompt[-file]` or `--append-system-prompt[-file]` content trips the same classifier even when the prompt is typed; the only stable workaround is to deliver BOTH the system prompt content AND the user prompt as a single typed user message, with the system content explicitly framed as user-provided context via `<system_context>` tags. Empirically reproducible across haiku-4-5 / sonnet-4-6 / opus-4-7 on Max-plan OAuth accounts; reference `smithersai/claude-p` uses typed-injection (D26) but does not pass any custom system prompt, sidestepping the D27 finding entirely. The model's behavior in response to the bundled message is functionally identical to receiving the same content via the system-role channel; the only difference is conversation-role attribution, which does not affect pi's expected outputs.
