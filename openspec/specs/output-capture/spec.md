...
unchanged. The mechanism for enforcing schema-constrained structured output is a
forced MCP tool-call on the claude-p driver path: the capture tool is advertised
as the sole MCP tool on a dedicated, isolated claude-p spawn, all native tools
are disallowed, the shim validates arguments against the capture tool's JSON
schema at the MCP protocol boundary, and the validated arguments are harvested
via IPC (see `mcp-stdio-shim` and `claude-p-driver`).

**Capture-path system prompt fidelity:** the capture path forwards
`ctx.systemPrompt` verbatim per constitution V. No capture-only system-prompt
addendum is appended. Model steering relies on: (a) the sole-tool advertisement
on the MCP shim (no other tool is callable), (b) the deterministic shim response
("Capture received. End your turn now."), and (c) the disallow-set blocking any
native-tool alternative. If the model emits text alongside the tool call, the
text is ignored.
...
- **THEN** the claude-p spawn's system prompt (delivered via `--system-prompt`/`--system-prompt-file`) equals that string verbatim
...
- **THEN** the delivered prompt contains a representation of all three messages (using `buildColdStartPrompt`)
...
- **THEN** the bridge spawns the capture claude-p subprocess with that system prompt
...
The capture path's pi-ai event stream SHALL emit exactly: one `start` event when the capture spawn opens, then a single terminal `done(toolUse)` or `error` event at finalization. The capture path SHALL NOT push intermediate `text_start`, `text_delta`, `text_end`, `thinking_start`, `thinking_delta`, `thinking_end`, `toolcall_start`, `toolcall_delta`, or `toolcall_end` events. This preserves pi-ai's block-lifecycle invariant (every `*_delta` paired with a `*_start`/`*_end` indexed against `partial.content`).
...
#### Scenario: Capture path subprocess is cleaned up
- **WHEN** a capture call completes
- **THEN** the capture call's claude-p subprocess and shim are no longer producing output or holding resources (reaped or torn down)
